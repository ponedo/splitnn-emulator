...
per_setup_time = 0
data["per_setup_time"] = np.copy(data["total_time"])
for i in range(sample_num):
    per_setup_time = data["per_setup_time"][i]
    data["per_setup_time"][i] = per_setup_time
data["linear"] = np.copy(data["total_time"])
sample_x = sample_num // 100
K = data["per_setup_time"][sample_x] / sample_x
for i in range(sample_num):
    data["linear"][i] = K * i

# Output each curve's data to a separate CSV file
for curve_name in ["total_time"]:
    x_key = f"x"
    y_key = curve_name
    if proportion_threshold is not None:
...
plt.plot(data["x"], data["per_setup_time"], label="accumulated node setup time")
...
# Save the figure
if proportion_threshold is not None:
    output_filename = f"link_setup_time_outlier_w{window_size}_r{int(args.outlier_tolerance_offset_ratio)}.png"
else:
