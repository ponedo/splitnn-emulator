...
for filename in sorted(os.listdir(folder_path)):
    if filename.startswith("kill.log.") and filename.split('.')[-1].isdigit():
        ...
        data["kill_time"].append(kill_time)
...
if proportion_threshold is not None:
    data["x"], data["kill_time"] = remove_outliers_with_window(data["x"], data["kill_time"], proportion_threshold, window_size)
else:
    data["x"] = data["x"]
...
acc_clean_time = 0
data["acc_clean_time"] = np.copy(data["kill_time"])
for i in range(sample_num):
    acc_clean_time += data["acc_clean_time"][i]
    data["acc_clean_time"][i] = acc_clean_time
data["linear"] = np.copy(data["kill_time"])
sample_x = sample_num // 100
K = data["acc_clean_time"][sample_x] / sample_x
for i in range(sample_num):
    data["linear"][i] = K * i

# Output each curve's data to a separate CSV file
for curve_name in ["acc_clean_time"]:
    x_key = f"x"
    y_key = curve_name
    if proportion_threshold is not None:
...
plt.plot(data["x"], data["acc_clean_time"], label="accumulated node clean time")
plt.plot(data["x"], data["linear"], '--', label="linear time")

# Customize the plot
plt.xlabel("node i")
plt.ylabel("Time (ms)")
plt.title(f"Accumulated clean time per node")
plt.legend()
plt.grid(True)

# Save the figure
if proportion_threshold is not None:
    output_filename = f"acc_clean_time_outlier_w{window_size}_r{int(args.outlier_tolerance_offset_ratio)}.png"
else:
    output_filename = "acc_clean_time_original.png"
